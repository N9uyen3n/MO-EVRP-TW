...
AddResult ParetoArchive::tryAdd(const Solution &newSolution) {
  ...
  // If there is ANY solution in the archive with FEWER vehicles, this new solution
  // is globally dominated and should not be added.
...
Solution ParetoArchive::getBestSolutionForObjective(int objectiveIndex) const {
  ...
  auto compareFn = [&](const Solution &a, const Solution &b) {
    ...
    case 1:
      return a.getWorkloadGini() <
             b.getWorkloadGini(); // Objective: Workload Variance
    case 2:
...
double ParetoArchive::computeHypervolume() const {
  ...
  // Update the fixed reference box if a new vehicle level is reached
  ...
  for (const auto &sol : flat) {
    ...
    if (sol.getTotalVehicles() > currentMinVehicles_)
      continue;
...
void ParetoArchive::updateReferencePoints() const {
  const auto &flat = getFront();
  if (flat.empty())
    return;

  // Find min vehicles of current front
  int minVeh = 1e9;
  for (const auto &sol : flat) {
    minVeh = std::min(minVeh, sol.getTotalVehicles());
  }

  // Only snapshot when we reach a NEW vehicle level
  if (minVeh < currentMinVehicles_) {
    currentMinVehicles_ = minVeh;

    refIdealDist_ = 1e18;
    refIdealGini_ = 1e18;
    refIdealMaxTime_ = 1e18;
    refNadirDist_ = -1e18;
    refNadirGini_ = -1e18;
    refNadirMaxTime_ = -1e18;

    for (const auto &sol : flat) {
      if (sol.getTotalVehicles() == minVeh) {
        refIdealDist_ = std::min(refIdealDist_, sol.getTotalDistance());
        refIdealGini_ = std::min(refIdealGini_, sol.getWorkloadGini());
        refIdealMaxTime_ = std::min(refIdealMaxTime_, sol.getMaxTime());

        refNadirDist_ = std::max(refNadirDist_, sol.getTotalDistance());
        refNadirGini_ = std::max(refNadirGini_, sol.getWorkloadGini());
        refNadirMaxTime_ = std::max(refNadirMaxTime_, sol.getMaxTime());
      }
    }

    // Add 20% slack to Nadir to allow room for exploration without breaking
    // the 1.0 boundary immediately
    refNadirDist_ += std::max(1.0, (refNadirDist_ - refIdealDist_) * 0.20);
    refNadirGini_ += std::max(0.01, (refNadirGini_ - refIdealGini_) * 0.20);
    refNadirMaxTime_ +=
        std::max(1.0, (refNadirMaxTime_ - refIdealMaxTime_) * 0.20);
  }
}
