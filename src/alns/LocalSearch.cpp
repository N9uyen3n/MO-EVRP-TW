...
void LocalSearch::run(Solution& solution) {
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        ...
        // ⭐ OPTIMIZATION: Update search context once per iteration
        updateSearchContext(solution);
        
        // Distance optimization (most impactful)
        if (runDistanceOptimization(solution)) {
            improved = true;
            searchContext_.invalidate(); // ⭐ Invalidate after solution changes
        }
        ...
        // Vehicle reduction (more aggressive - every 3 iterations)
        if ((iter % 3 == 0 || !improved) && solution.getRoutes().size() > 1) {
...
void LocalSearch::evaluateMove(const Solution& solution, MoveDescriptor& move, const LocalSearchWeights& weights) {
    ...
    if (move.type == MoveType::INTER_RELOCATE) {
        ...
        double distance_removed = instance->getDistance(prev_node, nodeId) + instance->getDistance(nodeId, next_node)
...
bool LocalSearch::searchRelocate(Solution& solution, MoveDescriptor& outBestMove, const LocalSearchWeights& weights, const SearchContext& ctx) {
    const auto& routes = solution.getRoutes();
    int numRoutes = routes.size();

    // ⭐ SAFETY: Validate context before use
    if (!ctx.isValid || ctx.centroids.size() != numRoutes || ctx.neighborLists.size() != numRoutes) {
        ...
        for (int k = 0; k < nodesToCheck; ++k) {
            ...
            for (int r2 : neighborLists[r1]) {
                ...
                for (size_t j : candidatePositions) {
                    ...
                    tempMove.type = (r1 == r2) ? MoveType::INTRA_RELOCATE : MoveType::INTER_RELOCATE;
                    tempMove.routeIdx1 = r1; tempMove.nodeIdx1 = i;
                    tempMove.routeIdx2 = r2; tempMove.nodeIdx2 = j;
                    ...
                    // If the move is not possibly feasible or it increases distance, skip it.
                    // A non-negative delta means no improvement or worsening, so we skip.
                    if (!preEval.isFeasible || preEval.distanceDelta >= -1e-9) {
                        continue;
                    }
                    
                    // If the pre-check passes, then we perform the full, expensive evaluation.
                    // For INTER_RELOCATE, evaluateMove now has its own fast path, but this
                    // delta check is still valuable for INTRA_RELOCATE and as a distance filter.
                    ...
                    if (activeMove.eval.isFeasible && activeMove.eval.objectiveDelta < -1e-9) {
                        ...
                        return true;
                    } else if (!activeMove.eval.isFeasible && preEval.distanceDelta < -5.0) {
                        // 🔋 ENERGY BOOST: Move was good for distance but failed feasibility
                        // Try station-assisted insertion
                        auto& routes = solution.getRoutes();
...
bool LocalSearch::searchSwap(Solution& solution, MoveDescriptor& outBestMove, const LocalSearchWeights& weights, const SearchContext& ctx) {
    const auto& routes = solution.getRoutes();
    int numRoutes = routes.size();
...
bool LocalSearch::runChargingOptimization(Solution& solution) {
    ...
    // Strategy 1: Remove redundant charging stations (Greedy Cleanup)
    ...
    // 🔋 Strategy 2b: Strategic hub charging (Partial Charging Strategy)
    if (optimizeStrategicHubCharging(solution)) {
        improved = true;
    }
...
// This addresses the "Nearest Station Trap" where routes get locked to wrong stations
bool LocalSearch::searchStationSwap(Solution& solution) {
    ...
    for (auto& route : routes) {
        ...
        if (nodes.size() <= 3) continue; // Need at least depot + 1 customer + depot
        ...
        for (size_t i = 1; i < nodes.size() - 1; ++i) {
            ...
            if (node->getType() != NodeType::STATION) continue;
            ...
            for (int candidateId : stationIds) {
                if (candidateId == nodeId) continue; // Skip same station
                
                auto candidateStation = std::static_pointer_cast<Station>(instance->getNodeById(candidateId));
                ...
                if (candidateCentroidDist >= currentCentroidDist - EPSILON) continue;
                ...
                double distImprovement = route.getTotalDistance() - testRoute.getTotalDistance();
...
bool LocalSearch::optimizeChargingAmounts(Solution& solution) {
    ...
    for (int r = 0; r < routes.size(); ++r) {
        ...
        for (size_t i = 1; i < nodes.size() - 1; ++i) {
            if (instance->getNodeById(nodes[i])->getType() == NodeType::STATION) {
                double chargeAmount = (i < states.size()) ? states[i].chargeAmount : 0.0;
                stationInfos.push_back({i, chargeAmount});
            }
        }
        
        // NEW Strategy 1: Remove stations right after depot (position 1) or at depot location
        if (!stationInfos.empty()) {
            for (auto it = stationInfos.begin(); it != stationInfos.end(); ) {
                ...
                bool sameAsDepot = (std::abs(stationNode->getX() - depotNode->getX()) < 0.01 &&
        ...
        for (size_t i = 1; i < nodes.size() - 1; ++i) {
            if (instance->getNodeById(nodes[i])->getType() == NodeType::STATION) {
                double chargeAmount = (i < currentStates.size()) ? currentStates[i].chargeAmount : 0.0;
...
bool LocalSearch::runVehicleReduction(Solution& solution) {
    if (solution.getNumRoutes() < 2) return false;
...
bool LocalSearch::tryEliminateSmallestRoute(Solution& solution) {
    ...
    if (smallestIdx == -1 || minCustomers > 15) return false; // Increased from 8 to 15 for more aggressive elimination
    
    std::cout << "[EliminateRoute] Trying to eliminate Route " << routes[smallestIdx].getId() 
    ...
    for (int custId : customersToMove) {
        ...
        for (int r = 0; r < (int)newRoutes.size(); ++r) {
            ...
            double demand = std::dynamic_pointer_cast<Customer>(custNode)->getDemand();
            if (newRoutes[r].getTotalDemand() + demand > newRoutes[r].getVehicle()->getCapacity()) {
                continue;
            }
    ...
    if (failedCustomers.empty()) {
        ...
        for (auto& route : newRoutes) {
            ...
            if (route.isFeasible()) {
                ...
                std::cout << "[EliminateRoute] FAILED - Route became infeasible after reconstruction." << std::endl;
                return false;
            }
        }
        
        solution.evaluateRoutes();
        std::cout << "[EliminateRoute] SUCCESS! Reduced to " << solution.getNumRoutes() << " vehicles." << std::endl;
        return true;
    }
    ...
    if (!failedCustomers.empty() && failedCustomers.size() <= 3) {
        std::cout << "[EliminateRoute] Attempting station-assisted insertion for " 
                  << failedCustomers.size() << " remaining customers..." << std::endl;
    ...
    std::cout << "[EliminateRoute] Failed to place " << failedCustomers.size() << " customers." << std::endl;
    return false;
}

std::vector<size_t> LocalSearch::getTopKInsertionPositions(
    const Route& route,
    int nodeId,
    int topK
) const {
    ...
    for (size_t pos = 1; pos < nodes.size(); ++pos) {
        ...
        double distAfter = instance->getDistance(prevNodeId, nodeId) + instance->getDistance(nodeId, nodes[pos]);
        ...
        if (pos > states.size()) continue; // Should not happen
        
        double arrivalTime = states[pos - 1].departureTime + instance->getTime(prevNodeId, nodeId);

        if (arrivalTime > targetNode->getDueDate()) {
            timePenalty = 10000.0; // Trễ cửa sổ -> Rất xấu
        } else if (arrivalTime < targetNode->getReadyTime()) {
            timePenalty = (targetNode->getReadyTime() - arrivalTime) * 0.5; // Phải chờ -> Phạt nhẹ
        }
    ...
    std::partial_sort(candidates.begin(),
                     candidates.begin() + std::min((size_t)topK, candidates.size()),
                     candidates.end());
...
double LocalSearch::calculateEuclideanDistance(const RouteCentroid& c1, const RouteCentroid& c2) const {
...
bool LocalSearch::runSmartMultiRouteMerge(Solution& solution) {
    ...
    for (int i = 0; i < numRoutes; ++i) {
        for (int j = i + 1; j < numRoutes; ++j) {
            int sizeSum = routes[i].getCustomers().size() + routes[j].getCustomers().size();
            if (sizeSum > 35 || sizeSum == 0) continue; // Increased from 12 to allow larger merges

            double dist = calculateEuclideanDistance(
                computeCentroid(routes[i]),
    ...
    std::cout << "[SmartMerge] Selected R" << routes[r1_idx].getId() << " & R" << routes[r2_idx].getId() << " for merge." << std::endl;
    ...
    double oldTotalDist = routes[r1_idx].getTotalDistance() + routes[r2_idx].getTotalDistance();
    ...
    std::sort(customer_pool.begin(), customer_pool.end(), [&](int a, int b) {
        return instance->getNodeById(a)->getReadyTime() < instance->getNodeById(b)->getReadyTime();
    });

    if (customer_pool.size() < 2) {
        if (customer_pool.empty()) return false;
    ...
    while (!customer_pool.empty() && !construction_failed) {
        ...
        for (int cust_id : customer_pool) {
            for (int r_idx = 0; r_idx < 2; ++r_idx) {
                std::vector<size_t> candidatePos = getTopKInsertionPositions(newRoutes[r_idx], cust_id, 3);
        ...
        if (best_cust_id != -1) {
            ...
            customer_pool.erase(std::remove(customer_pool.begin(), customer_pool.end(), best_cust_id), customer_pool.end());
            fail_count = 0;
        } else {
            ...
            for (int r_idx = 0; r_idx < 2; ++r_idx) {
                if (newRoutes[r_idx].getCustomers().empty()) continue;
                ...
                if (stationId != -1) {
                    ...
                    InsertionResult res = newRoutes[r_idx].checkInsertionCost(stationId, insertPos);
            ...
            if (!inserted_station) {
                fail_count++;
                if (fail_count >= 5) construction_failed = true;
            }
        }
    }
    ...
    if (finalRoutes.size() < 2 || (finalRoutes.size() == 2 && newTotalDist < oldTotalDist * 0.99)) {
        ...
        if (finalRoutes.size() < 2) {
            ...
            std::cout << "[SmartMerge] OPTIMIZATION SUCCESS (Distance improved: " << (oldTotalDist - newTotalDist) << ")\n";
        }
        return true;
    }
...
void LocalSearch::updateSearchContext(const Solution& solution) {
    ...
    for (int r1 = 0; r1 < numRoutes; ++r1) {
        for (int r2 = 0; r2 < numRoutes; ++r2) {
            ...
            if (r1 == r2 || areRoutesClose(searchContext_.centroids[r1], 
                                          searchContext_.centroids[r2], 
                                          distanceThreshold_)) {
...
MoveEvaluation LocalSearch::evaluateRelocateDelta(const Solution& solution, const MoveDescriptor& move) {
    ...
    distanceDelta -= (instance->getDistance(r1_prev, nodeId) + instance->getDistance(nodeId, r1_next));
    ...
    if (isIntraMove) {
        ...
        distanceDelta += (instance->getDistance(r2_prev, nodeId) + instance->getDistance(nodeId, r2_next));
    } else {
    ...
    result.isFeasible = true; // It's POSSIBLY feasible. The full evaluateMove will confirm.
    result.distanceDelta = distanceDelta;
    result.objectiveDelta = distanceDelta; // For now, objective is just distance delta
 ...
 */
int LocalSearch::findPrecedingStation(const Route& route, size_t insertPos) const {
    const auto& nodes = route.getNodes();
 ...
 */
double LocalSearch::calculateEnergyGap(const Route& route, size_t position) const {
 ...
 */
bool LocalSearch::tryRelocateWithEnergyBoost(
    Route& routeFrom, int fromNodeIdx,
    ...
    if (nodeId < 0) return false;  // Invalid node
...
/**
 * Optimize charging at strategic "hub" stations.
 * Hub stations are those visited by multiple routes.
 */
bool LocalSearch::optimizeStrategicHubCharging(Solution& solution) {
    auto& routes = solution.getRoutes();
    bool improved = false;
    
    // Identify "hub" stations (stations that multiple routes pass through)
    std::unordered_map<int, int> stationUsage;
    ...
    // The current Route::evaluate() already optimizes charging amounts
    // This function could be enhanced to:
    // 1. Prefer full charging at hub stations
    // 2. Try alternative hub stations that might enable better route connections
    // 3. Remove non-hub stations if feasible
...
int LocalSearch::removeRedundantStations(Route& route) {
    ...
    // Loop until no more stations can be removed (to handle multiple redundant stations)
