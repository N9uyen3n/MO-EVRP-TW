...
InsertionResult Route::checkInsertionCost(int nodeId, size_t position) const {
  ...
  for (int i = n - 2; i >= 0; --i) {
    double energy_to_next =
        instance->getDistance(simNodeSequence[i], simNodeSequence[i + 1]) *
        vehicle->getEnergyConsumptionRate();
  ...
  double simTotalTime =
      simStates[n - 1].arrivalTime - simStates[0].departureTime;
...
double Route::getTotalDemand() const {
  ...
  // No need to call evaluate() here, as nodeSequence is independent of
  // evaluation results
  for (int nodeId : *nodeSequence) {
...
std::vector<int> Route::getRedundantStations() const {
  ...
  for (int i = 1; i < n - 1; ++i) {
    ...
    if (i >= 1 && i + 1 < n) { // Guard: boundary check
