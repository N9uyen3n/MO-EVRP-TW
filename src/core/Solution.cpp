...
double Solution::getGiniCoefficient() const {
...
void Solution::evaluateRoutes() {
    ...
    if (route_durations.size() > 1) {
        ...
        if (sum > 1e-4) {
            this->giniCoefficient = sum_abs_diff / (2.0 * route_durations.size() * sum);
        } else {
            this->giniCoefficient = 0.0;
        }
    } else {
...
void Solution::print() const {
    ...
    << " Total Time: " << getTotalTime() << " Max Time: " << getMaxTime() << std::endl;
    for (const auto& route : *routes) { // Use pointer
        route.print();
    }
}
