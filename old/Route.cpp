...
void Route::evaluate() const {
  ...
  // ── Backward pass: minBatteryReq[i] = min battery needed at node i ────────
  minBatteryReq.assign(n, 0.0);
  for (int i = n - 2; i >= 0; --i) {
    double e = instance->getDistance(nodeSequence[i], nodeSequence[i + 1]) * eRate;
    auto   nxt = instance->getNodeById(nodeSequence[i + 1]);
    if (nxt->getType() == NodeType::STATION)
  ...
  {
    auto depot = std::static_pointer_cast<Depot>(instance->getNodeById(0));
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    case NodeType::STATION: {
      // [OPT-4] Consecutive-same-station check inline
    ...
    case NodeType::DEPOT: {
      ...
      depot->setLastTime(to.arrivalTime);
      break;
    }
    }
  }
  ...
  isDirty          = false;
  absorptionDirty_ = true; // absorption always re-runs after a new forward pass
}
  ...
  for (int i = 1; i < n; ++i) {
    ...
    for (int k = s + 1; k <= i; ++k) {
      ...
      if (node_k->getType() == NodeType::CUSTOMER) {
        auto cust_k    = std::static_pointer_cast<Customer>(node_k);
...
InsertionResult Route::checkInsertionCost(int nodeId, size_t position) const {
  ...
  for (int i = n - 2; i >= 0; --i) {
    ...
    auto   nxt  = instance->getNodeById(simNodeAt(i + 1));
    if (nxt->getType() == NodeType::STATION)
...
std::vector<int> Route::getCustomers() const {
  ...
  for (int nid : nodeSequence)
    if (instance->getNodeById(nid)->getType() == NodeType::CUSTOMER)
...
double Route::getTotalDemand() const {
  ...
  double total = 0.0;
  for (int nid : nodeSequence)
    total += instance->getNodeById(nid)->getDemand();
...
double Route::getCentroidX() const {
  ...
  for (int nid : nodeSequence) {
    auto node = instance->getNodeById(nid);
    if (node->getType() == NodeType::CUSTOMER) { sumX += node->getX(); ++count; }
  }
...
double Route::getCentroidY() const {
  ...
  for (int nid : nodeSequence) {
    auto node = instance->getNodeById(nid);
    if (node->getType() == NodeType::CUSTOMER) { sumY += node->getY(); ++count; }
  }
...
bool Route::canPossiblyInsert(int nodeIdToInsert, size_t position,
  ...
  if (nodeIdToRemove != -1) {
    auto nr = instance->getNodeById(nodeIdToRemove);
...
std::vector<double> Route::getTimeSlack() const {
  ...
  auto lastNode = instance->getNodeById(nodeSequence[n - 1]);
...
std::vector<int> Route::getRedundantStations() const {
  ...
  const double EPSILON = 1e-9;
  for (int i = 1; i < n - 1; ++i) {
    if (instance->getNodeById(nodeSequence[i])->getType() != NodeType::STATION)
      continue;
