...
Instance::Instance(const std::vector<std::shared_ptr<Node>> &nodes,
                   double vehicleCapacity, double vehicleBattery,
                   double vehicleEnergyRate, double vehicleVelocity)
    : nodes(nodes), vehicleCapacity(vehicleCapacity),
      vehicleBattery(vehicleBattery), vehicleEnergyRate(vehicleEnergyRate),
  ...
  for (auto &node : nodes) {
    ...
    if (auto customer = std::dynamic_pointer_cast<Customer>(node)) {
      if (maxDemand < customer->getDemand())
        maxDemand = customer->getDemand();
  ...
  nodeTypeCache_.resize(maxId + 1);
  for (const auto &node : nodes)
    nodeTypeCache_[node->getId()] = node->getType();
}
...
// getDistance and getTime are defined inline in Instance.h
// Defining them again here would cause ODR violation — removed.

double Instance::getMaxDemand() const { return this->maxDemand; }

double Instance::getVehicleCapacity() const { return vehicleCapacity; }

double Instance::getVehicleBattery() const { return vehicleBattery; }

double Instance::getVehicleEnergyRate() const { return vehicleEnergyRate; }

double Instance::getVehicleVelocity() const { return vehicleVelocity; }
...
const std::shared_ptr<Node> &Instance::getNodeById(int id) const {
  if (id >= 0 && id < nodes.size()) {
    // Assuming node IDs are contiguous and 0-based, which is how they are
    // parsed.
    return nodes[id];
  }
  throw std::runtime_error("Node with ID " + std::to_string(id) +
                           " not found or ID is out of bounds.");
}

std::string Instance::toString() const {
  std::stringstream ss;
  ss << "Capacity: " << vehicleCapacity << ", Battery: " << vehicleBattery
     << ", Energy rate: " << vehicleEnergyRate << "Velocity" << vehicleVelocity;
  return ss.str();
}