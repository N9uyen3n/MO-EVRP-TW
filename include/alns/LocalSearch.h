...
private:
  ...
  struct SearchContext {
    ...
    void markDirty(int r1, int r2 = -1) {
      if (r1 >= 0 && r1 < (int)rankingDirty.size()) rankingDirty[r1] = true;
      if (r2 >= 0 && r2 < (int)rankingDirty.size()) rankingDirty[r2] = true;
    }
  };
  ...
  static constexpr int    EARLY_STOP_THRESHOLD   = 6;
  static constexpr int    CHARGING_FREQUENCY     = 4;
  // VEHICLE_REDUCTION_FREQUENCY removed — vehicle reduction runs every iter now
