...
private:
  ...
  // dirtyFrom_: vị trí đầu tiên trong nodeSequence bị ảnh hưởng bởi mutator
  // gần nhất. states[0..dirtyFrom_-1] vẫn hợp lệ khi isDirty == true.
  // Luôn được set khi isDirty được set.
  mutable int dirtyFrom_ = 0;

  // absorptionFrom_: lưu giá trị dirtyFrom_ tại thời điểm evaluate() hoàn tất,
  // trước khi dirtyFrom_ bị reset về 0. Được dùng bởi runAbsorptionIfNeeded()
  // để xác định điểm bắt đầu incremental. Reset về 0 sau khi absorption xong.
  mutable int absorptionFrom_ = 0;
