...
public:
  ...
  // Returns time slack at each node: max(0, dueDate - departureTime)
  std::vector<double> getTimeSlack() const;

  // Returns energy slack (remaining - minRequired) at each node. High =
  // flexible, Low = tight.
  std::vector<double> getEnergySlack() const;
  // Returns (position, slack) pairs for nodes with slack < thresholdRatio *
  // batteryCapacity
  std::vector<std::pair<int, double>>
  getBottleneckNodes(double thresholdRatio = 0.15) const;
