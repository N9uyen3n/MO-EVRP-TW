...
void RegretKRepair::execute(Solution& solution, const std::vector<int>& unservedCustomers, std::mt19937& rng) {
    ...
    // ⭐ ADAPTIVE K: Adjust K based on problem size
    int numRoutes = solution.getNumRoutes();
    int numRemaining = remainingCustomers.size();
    
    // Calculate adaptive K
    int adaptiveK = k_regret; // Start with configured K
    
    if (numRemaining > 20) {
        // Many customers remaining: use higher K for better quality
        adaptiveK = std::min(5, numRoutes);
    } else if (numRemaining < 5) {
        // Few customers remaining: use lower K for speed
        adaptiveK = std::min(2, numRoutes);
    } else {
        // Medium: use configured K
        adaptiveK = std::min(k_regret, numRoutes);
    }
    ...
    while (!remainingCustomers.empty()) {
        ...
        for (int custId : remainingCustomers) {
            ...
            std::vector<InsertionCost> kBest = findKBestInsertions(custId, solution, rng, adaptiveK);
            ...
            if (kBest.size() >= adaptiveK) {
                for(size_t i = 1; i < adaptiveK; ++i) {
...
std::vector<RegretKRepair::InsertionCost> RegretKRepair::findKBestInsertions(int customerId, Solution& solution, std::mt19937& rng, int adaptiveK) {
