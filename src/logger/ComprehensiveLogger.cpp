...
void ComprehensiveLogger::initProgressFile() {
    progressFile << "Iteration,Timestamp_ms,ArchiveSize,"
                 << "BestDistance,BestVehicles,BestMaxTime,BestWorkloadVar\n";
}
...
void ComprehensiveLogger::initFrontObjectiveFile() {
    frontObjFile << "SolutionID,TotalVehicles,TotalDistance,WorkloadVariance,MaxTime,IsFeasible\n";
}
...
void ComprehensiveLogger::logProgress(int iteration, long long time_ms, const ParetoArchive& archive) {
    ...
    if (archive.getSize() > 0) {
        for (const auto& sol : archive.getFront()) {
            ...
            if (sol.getWorkloadVariance() < bestWorkloadVar) bestWorkloadVar = sol.getWorkloadVariance();
        }
    }
...
void ComprehensiveLogger::logOperatorSegment(int segment, const alns::OperatorPool& destroyPool, const alns::OperatorPool& repairPool) {
...
void ComprehensiveLogger::logFinalFront(const ParetoArchive& archive) {
    ...
    for (const auto& sol : archive.getFront()) {
        frontObjFile << solutionID << "," << sol.getTotalVehicles() << ","
                     << sol.getTotalDistance() << "," << sol.getWorkloadVariance() << ","
                     << sol.getMaxTime() << ","
