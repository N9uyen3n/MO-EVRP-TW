// src/logging/ComprehensiveLogger.cpp
#include "logger/ComprehensiveLogger.h"
#include "alns/ALNSSolver.h"    // Cần cho ALNSConfig, OperatorPool
...
#include <limits>
#include <numeric>
#include <filesystem> // Cần C++17
#include <ctime>

namespace logging {
namespace fs = std::filesystem;

// --- 1. Constructor & Destructor (Đổi tên) ---

ComprehensiveLogger::ComprehensiveLogger(const std::string& outputDirectory, const std::string& runName) {
    try {
    ...
    if (!configFile.is_open() || !progressFile.is_open() || !operatorFile.is_open() ||
        !evolutionLogFile.is_open() || !frontObjFile.is_open() ||
        !frontDetailFile.is_open() || !summaryFile.is_open()) {
        std::cerr << "FATAL ERROR: Logger could not open all output files." << std::endl;
    }
...
ComprehensiveLogger::~ComprehensiveLogger() {
    ...
    if (operatorFile.is_open()) operatorFile.close();
    if (evolutionLogFile.is_open()) evolutionLogFile.close();
    if (frontObjFile.is_open()) frontObjFile.close();
    if (frontDetailFile.is_open()) frontDetailFile.close();
    if (summaryFile.is_open()) summaryFile.close();
}

// --- 2. Các hàm ghi Header (Init) (Đổi tên) ---
...
void ComprehensiveLogger::initProgressFile() {
    progressFile << "Iteration,Timestamp_ms,ArchiveSize,"
                 << "BestDistance,BestVehicles,BestMaxTime,BestEnergy\n";
}
...
void ComprehensiveLogger::initEvolutionLogFile() {
    ...
    evolutionLogFile << "(Logs all accepted non-dominated/dominating solutions)\n\n";
}

void ComprehensiveLogger::initFrontObjectiveFile() {
    frontObjFile << "SolutionID,TotalDistance,TotalVehicles,TotalTime,"
                 << "MaxTime,TotalEnergy,IsFeasible\n";
}
...
// --- 3. Các hàm Log chính (Đổi tên) ---

void ComprehensiveLogger::logConfig(const ALNSConfig& config) {
...
void ComprehensiveLogger::logProgress(int iteration, long long time_ms, const ParetoArchive& archive) {
    if (!progressFile.is_open()) return;
    // (Logic y hệt như file Logger.cpp tôi đã viết trước đó)
    ...
    if (archive.getSize() > 0) {
        for (const auto& sol : archive.getFront()) {
            if (sol.getTotalDistance() < bestDist) bestDist = sol.getTotalDistance();
            if (sol.getTotalVehicles() < bestVeh) bestVeh = sol.getTotalVehicles();
            if (sol.getMaxTime() < bestMaxTime) bestMaxTime = sol.getMaxTime();
            if (sol.getTotalEnergy() < bestEnergy) bestEnergy = sol.getTotalEnergy();
        }
    }
...
void ComprehensiveLogger::logOperatorSegment(int segment, const OperatorPool& destroyPool, const OperatorPool& repairPool) {
    if (!operatorFile.is_open()) return;
    // (Logic y hệt như file Logger.cpp tôi đã viết trước đó)
...
void ComprehensiveLogger::logEvolutionStep(int iteration, const std::string& destroyOp, const std::string& repairOp, const std::string& result, const Solution& newSolution) {
    if (!evolutionLogFile.is_open()) return;
    // (Logic y hệt như file Logger.cpp tôi đã viết trước đó)
    evolutionLogFile << "----------------------------------------\n"
    ...
    evolutionLogFile.flush(); // Đảm bảo ghi ngay lập tức
}

void ComprehensiveLogger::logFinalFront(const ParetoArchive& archive) {
    if (!frontObjFile.is_open() || !frontDetailFile.is_open()) return;
    // (Logic y hệt như file Logger.cpp tôi đã viết trước đó)
    int solutionID = 1;
    for (const auto& sol : archive.getFront()) {
        frontObjFile << solutionID << "," << sol.getTotalDistance() << ","
                     << sol.getTotalVehicles() << "," << sol.getTotalTime() << ","
                     << sol.getMaxTime() << "," << sol.getTotalEnergy() << ","
...
void ComprehensiveLogger::logSummary(long long total_ms, int total_iter, int final_archive_size) {
