...
BenchmarkResult runInstance(const std::string& instancePath, const alns::ALNSConfig& config) {
    ...
    try {
        ...
        if (!paretoFront.empty()) {
            ...
            std::cout << "  Best: Veh=" << result.bestVehicles 
                      << ", Dist=" << std::fixed << std::setprecision(2) << result.bestDistance << "\n";
        } else {
            result.feasible = false;
...
void writeResultsToCSV(const std::vector<BenchmarkResult>& results, const std::string& filename) {
    ...
    // Header
    file << "Instance,Customers,Stations,Time(ms),ParetoSize,Vehicles,Distance,Workload,MaxTime,Feasible\n";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[INFO] Found " << instanceFiles.size() << " instances in " << dataDir << "\n\n";
    ...
    long long totalTime = std::chrono::duration_cast<std::chrono::seconds>(benchmarkEnd - benchmarkStart).count();
    ...
    for (const auto& r : results) {
        std::cout << std::left << std::setw(20) << r.instanceName
                  << std::setw(8) << r.customers
                  << std::setw(10) << r.timeMs
                  << std::setw(6) << r.bestVehicles
                  << std::setw(12) << std::fixed << std::setprecision(2) << r.bestDistance
