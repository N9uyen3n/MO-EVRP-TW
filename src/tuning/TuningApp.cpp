 ...
 *
 * Cách biên dịch:
 *   - Thêm vào CMakeLists.txt: add_executable(TuningApp
 * src/tuning/TuningApp.cpp)
 *
 * Cách chạy:
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cerr << "FATAL: Could not parse instance file " << instancePath
  ...
  // Lấy các giá trị từ parser, nếu không có thì dùng giá trị mặc định của
  // config
  config.maxIterations =
      parser.get<int>("--maxIterations", config.maxIterations);
  config.segmentIterations =
      parser.get<int>("--segmentIterations", config.segmentIterations);
  config.hvImprovementThreshold = parser.get<double>(
      "--hvImprovementThreshold", config.hvImprovementThreshold);
  config.hvStagnationLimit =
      parser.get<int>("--hvStagnationLimit", config.hvStagnationLimit);

  config.decayParameter =
      parser.get<double>("--decayParameter", config.decayParameter);
  config.scoreDominating =
      parser.get<double>("--scoreDominating", config.scoreDominating);
  config.scoreNonDominated =
      parser.get<double>("--scoreNonDominated", config.scoreNonDominated);
  config.scoreDominated =
      parser.get<double>("--scoreDominated", config.scoreIdentical);
  config.scoreIdentical =
      parser.get<double>("--scoreIdentical", config.scoreIdentical);
  ...
  config.noiseParameter =
      parser.get<double>("--noiseParameter", config.noiseParameter);

  config.useLocalSearch = !parser.exists(
      "--no-local-search"); // Bật trừ khi có cờ --no-local-search
  config.localSearchIntensity =
      parser.get<int>("--localSearchIntensity", config.localSearchIntensity);

  config.startTemperature =
      parser.get<double>("--startTemperature", config.startTemperature);
  config.coolingRate = parser.get<double>("--coolingRate", config.coolingRate);
  config.minTemperature =
      parser.get<double>("--minTemperature", config.minTemperature);

  // [NEW] Lấy các tham số logging. Logging sẽ chỉ được bật nếu cả 2 tham số này
  // được cung cấp.
  ...
  auto solver =
      std::make_unique<alns::ALNSSolver>(instance, config, outputDir, runName);

  std::vector<Solution> paretoFront;
  try {
    paretoFront = solver->solve();
  } catch (const std::exception &e) {
    std::cerr << "FATAL: An exception occurred during solve(): " << e.what()
              << std::endl;
    return 1;
  }
  ...
  if (config.enableLogging) {
    std::cout << "--- Run with logging enabled. See output files in "
              << outputDir << " ---" << std::endl;
  }
