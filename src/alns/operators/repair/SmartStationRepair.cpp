    ...
    for (size_t r = 0; r < routes.size(); ++r) {
      ...
      for (size_t i = 1; i < nodes.size();
        ...
        if (result.isFeasible) {
          ...
          // Customer -> Next) Relevant station: Nearest to PrevNode
          int stationId = instance->getNearestStationId(prevNodeId);
          if (stationId != -1) {
            // Avoid redundant stations: If prevNode is already THIS station,
            // don't add
          ...
          // Station -> Next) Relevant station: Nearest to Customer or NextNode
          // Usually we need energy TO reach the customer, so B1 is more common
          // for "reaching" the customer. But B2 is useful if we reach customer
          // but can't reach Next.
          stationId = instance->getNearestStationId(customerId);
          if (stationId != -1) {
            // Insert Customer at i, Station at i+1
            double costBuf = evaluateInsertionWithStation(route, customerId, i,
                                                          stationId, i + 1);
            if (costBuf < bestInsertion.costIncrease) {
              bestInsertion.customerId = customerId;
              bestInsertion.routeIndex = static_cast<int>(r);
    ...
    if (bestInsertion.routeIndex != -1) {
      ...
      if (bestInsertion.requiresStation) {
        ...
        if (bestInsertion.stationPosition == bestInsertion.position) {
          ...
          // Insert Customer first at pos, then Station at pos (pushing customer
          // to pos+1) Wait: logic: Original: A (i-1), B (i) Goal: A, Station,
          // Customer, B
