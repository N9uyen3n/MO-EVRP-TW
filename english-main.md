...
#### 3.4.4. Vehicle Reduction

Attempts to merge small routes to reduce the number of vehicles used.

**Algorithm 21: runVehicleReduction**
```pseudocode
1.  Require: current_solution
2.  Ensure: improved (boolean)
3.
4.  centroids <- computeAllCentroids(solution)
5.  routes <- solution.getRoutes()
6.  Sort routes by number of customers (ascending)
7.
8.  for each route r_1 in routes do
9.      if |r_1.customers| > 3 then
10.         continue // Only merge small routes
11.     end if
12.
13.     for each route r_2 in routes where r_2 != r_1 do
14.         // Granular Filter
15.         if not areRoutesClose(centroids[r_1], centroids[r_2], distanceThreshold) then
16.             continue
17.         end if
18.
19.         // Try merging r1 into r2
20.         testSolution <- solution.copy()
21.         if mergeRoutes(testSolution, r_2, r_1) then
22.             solution <- testSolution
23.             return true // First Improvement
24.         end if
25.     end for
26. end for
27. return false
