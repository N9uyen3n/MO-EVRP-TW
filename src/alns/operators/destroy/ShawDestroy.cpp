...
ShawDestroy::ShawDestroy(std::shared_ptr<Instance> instance, int determinism_param)
    : instance(instance), determinism(determinism_param) {}

std::string ShawDestroy::getName() const {
    return "Shaw Removal (Related)";
}
...
std::vector<int> ShawDestroy::execute(Solution& solution, int nodesToRemove, std::mt19937& rng) {
    ...
    if (allCustomers.empty()) return removed;
    ...
    int seedIdx = distIdx(rng);
    int seedCustId = allCustomers[seedIdx];
    
    removed.push_back(seedCustId);
    
    // Xóa seed khỏi danh sách khả dụng
    allCustomers.erase(allCustomers.begin() + seedIdx);
    ...
    while (removed.size() < nodesToRemove && !allCustomers.empty()) {
        ...
        struct RelatedNode {
            ...
            bool operator<(const RelatedNode& other) const { return rank < other.rank; }
        };
        std::vector<RelatedNode> candidates;

        for (int cId : allCustomers) {
            double R = calculateRelatedness(rId, cId, custToRoute, custToDemand, w_dist, w_time, w_demand, w_route, w_station);
        ...
        int idx = static_cast<int>(candidates.size() * std::pow(d(rng), determinism));
        ...
        if (it != allCustomers.end()) allCustomers.erase(it);
    }
    ...
    for (auto& route : routes) {
        ...
        for (size_t i=1; i < oldNodes.size()-1; ++i) {
            bool del = false;
            for(int rm : removed) if(rm == oldNodes[i]) { del=true; break; }
...
double ShawDestroy::calculateRelatedness(int cust1_id, int cust2_id,
                            ...
                            double w_dist, double w_time, double w_demand, double w_route, double w_station) {
    
    // Khoảng cách chuẩn hóa (tương đối)
    double dist = instance->getDistance(cust1_id, cust2_id) / instance->getMaxDistance();
    
    // Chênh lệch thời gian phục vụ
    double timeDiff = std::abs(instance->getNodeById(cust1_id)->getReadyTime() - 
                               instance->getNodeById(cust2_id)->getReadyTime()) / instance->getMaxTimeWindow();
    
    // Chênh lệch nhu cầu
    double demandDiff = std::abs(custToDemand.at(cust1_id) - custToDemand.at(cust2_id)) / instance->getMaxDemand();

    // Khác tuyến hay cùng tuyến
    int routeDiff = (custToRoute.at(cust1_id) == custToRoute.at(cust2_id)) ? 0 : 1;
    ...
    return w_dist * dist + w_time * timeDiff + w_demand * demandDiff + w_route * routeDiff + w_station * stationDiff;
}