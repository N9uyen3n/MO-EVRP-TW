...
void AdaptiveInsertion::execute(Solution &solution,
                                ...
                                std::mt19937 &rng) {
  // 1. Select mode biased by weight hint (Fix 2)
  std::uniform_real_distribution<> dist(0.0, 1.0);
  double r = dist(rng);

  Mode mode;
  if (r < weightHint_.dist)
    mode = Mode::DISTANCE_FOCUSED;
  else if (r < weightHint_.dist + weightHint_.gini)
    mode = Mode::WORKLOAD_FOCUSED;
  else
  ...
  for (int customerId : customers) {
    ...
    for (size_t r = 0; r < routes.size(); ++r) {
      ...
      if (!route.quickCapacityCheck(demand))
        continue;
      ...
      for (size_t i = 1; i < nodes.size(); ++i) {
        ...
        if (result.isFeasible) {
          double cost = calculateCost(result, mode, route, meanRouteDuration,
                                      maxRouteDuration, customerId);
          if (cost < bestInsertion.costIncrease) {
            bestInsertion = {customerId, static_cast<int>(r),
                             static_cast<int>(i), cost};
          }
        }
      }
    }
...
double AdaptiveInsertion::calculateCost(const InsertionResult &result,
  ...
  case Mode::WORKLOAD_FOCUSED: {
    // Fix 1: Use deltaTime instead of deltaDistance for time estimation
  ...
  case Mode::MAXTIME_FOCUSED: {
    // Fix 1: Use deltaTime instead of deltaDistance
    double newRouteTime = route.getTotalTime() + result.deltaTime;

    double cost = result.deltaDistance;

    // Fix 3: Normalized penalty instead of magic numbers
    if (maxRouteDuration > 0) {
      if (newRouteTime > maxRouteDuration) {
        // Penalty proportional to overrun ratio, scaled by typical cost
        // magnitude
        double overrunRatio =
            (newRouteTime - maxRouteDuration) / maxRouteDuration;
        cost += overrunRatio * 100.0 * (std::abs(result.deltaDistance) + 1.0);
      } else if (newRouteTime > 0.95 * maxRouteDuration) {
        // Softer warning penalty: proportional to how close to max
        double proximityRatio = (newRouteTime - 0.95 * maxRouteDuration) /
                                (0.05 * maxRouteDuration);
        cost += proximityRatio * 10.0 * (std::abs(result.deltaDistance) + 1.0);
      }
    }
    return cost;
  }
