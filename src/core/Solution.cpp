...
Solution::Solution(const std::shared_ptr<Instance> &instance)
    : instance(instance),
      routes(std::make_shared<std::vector<Route>>()), // Initialize with a new
                                                      // vector
      totalVehicles(0), totalDistance(0.0), workloadVariance(0.0),
      giniCoefficient(0.0), totalTime(0.0), maxTime(0.0), feasible(true) {}
...
double Solution::getWorkloadVariance() const { return this->workloadVariance; }

double Solution::getGiniCoefficient() const { return this->giniCoefficient; }
...
bool Solution::isFeasible() const { return checkGlobalFeasibility(); }
...
void Solution::evaluateRoutes() {
  ...
  // Calculate Workload Variance & Gini Coefficient
  if (route_durations.size() > 1) {
    ...
    // 2. Gini Coefficient: G = sum(|xi - xj|) / (2 * n * sum(xi))
    double sum_abs_diff = 0.0;

    for (size_t i = 0; i < route_durations.size(); ++i) {
      sq_sum += std::pow(route_durations[i] - mean, 2);

      for (size_t j = 0; j < route_durations.size(); ++j) {
        sum_abs_diff += std::abs(route_durations[i] - route_durations[j]);
      }
    }
    ...
    if (sum > 1e-4) {
      this->giniCoefficient =
          sum_abs_diff / (2.0 * route_durations.size() * sum);
    } else {
      this->giniCoefficient = 0.0;
    }
  } else {
...
double Solution::getAverageRouteTime() const {
  ...
  for (const auto &route : *routes) {
    if (route.size() > 2) { // Not empty (has customers)
...
void Solution::print() const {
  std::cout << "Total Vehicles: " << getTotalVehicles()
            ...
            << " Workload Var: " << getWorkloadVariance()
            << " Gini: " << getGiniCoefficient() // ⭐ NEW
