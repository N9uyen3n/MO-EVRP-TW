...
UnifiedCostDestroy::UnifiedCostDestroy(std::shared_ptr<Instance> instance,
...
std::vector<int> UnifiedCostDestroy::execute(Solution &solution,
                                             int nodesToRemove,
                                             std::mt19937 &rng) {
  ...
  for (const auto &route : routes) {
    ...
    for (size_t i = 1; i < nodes.size() - 1; ++i) {
      ...
      double baseSaving =
          calculateNodeSaving(route, i, metric, avgRouteDuration);
  ...
  // Enhanced with RCWPS/RCWSS: also remove adjacent stations with 50%
  // probability
  ...
  // First pass: identify adjacent stations to also remove (RCWPS/RCWSS)
  // Key insight: only remove a station if it truly serves the customer being
  // removed. If the station's other neighbor is a non-removed customer, the
  // station may be needed for that customer's energy arc → skip removal.
  ...
  for (const auto &route : routes) {
    ...
    for (size_t i = 1; i < nodes.size() - 1; ++i) {
      ...
      // RCWPS: Check preceding node (i-1) is a station, skip if i-1 == 0
      // (depot)
      if (i > 1 &&
        ...
        if (safeToRemove && coinFlip(rng) < 0.5) {
          stationsToRemove.insert(nodes[i - 1]);
        }
      }

      // RCWSS: Check succeeding node (i+1) is a station, skip if last node
      // (depot)
      if (i + 1 < nodes.size() - 1 &&
          instance->getNodeById(nodes[i + 1])->getType() == NodeType::STATION) {
        // Check if station's OTHER neighbor (i+2) is a non-removed customer
...
double UnifiedCostDestroy::calculateNodeSaving(const Route &route,
                                               size_t position,
                                               CostMetric metric,
                                               double avgRouteDuration) {
  const auto &nodes = route.getNodes();
  ...
  if (metric == DISTANCE) {
    double distWith =
        instance->getDistance(prev, curr) + instance->getDistance(curr, next);
    double distWithout = instance->getDistance(prev, next);
    return distWith - distWithout;
  } else if (metric == TIME) {
    // Includes travel time + service time
    double timeWith =
        instance->getTime(prev, curr) + instance->getTime(curr, next);
...
double UnifiedCostDestroy::getTimeWindowSlack(int customerId,
                                              const Route &route) {
