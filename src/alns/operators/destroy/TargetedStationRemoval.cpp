...
std::vector<int> TargetedStationRemoval::execute(Solution &solution,
  ...
  // we scale it down. The actual cap is applied after collecting candidates.
  ...
  // 1.5 Cap the number of stations to remove based on target
  int stationsToRemove = std::max(1, std::min(nodesToRemove, (int)candidates.size()));
...
double TargetedStationRemoval::calculateStationScore(const Route &route,
  ...
  // --- TIEBREAKER & WORST-CHARGE SCORING ---

  // 1. Low Usage & Worst-Charge
  double chargeAmount = states[nodeIdx].chargeAmount;
  double capacity = route.getVehicle()->getBatteryCapacity();
  double arrivalBattery = states[nodeIdx].remainingBattery - chargeAmount;

  if (arrivalBattery > 0.5 * capacity) {
    score += 150.0 * (arrivalBattery / capacity);
  }
